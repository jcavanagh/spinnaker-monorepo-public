...
export function generatePrBody(results: MergeResult[]) {
  ...
  for (const result of results) {
    if (!result.isClean) {
      ...
      body += `<br><br>Merge failed for ${result.repo} - run the following command, resolve conflicts, and push:<br>`;
      ...
      body += '```';
      body += `<details><summary>Output</summary>${result.exec.err}</details>`;
    }
  }

  // Collect successful merges into one line
  const successes = results.filter((it) => it.isClean);
  body += `<br><br>`;
...
export async function setup() {
  ...
  console.log(existingMergeBranch);
