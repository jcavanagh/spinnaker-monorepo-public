...
function fetchBuildTags(prefix, callback) {
    request('GET', `/repos/${env.GITHUB_REPOSITORY}/git/refs/tags/${prefix}build-number-`, null, (err, status, result) => {
        ...
        if (status === 404) {
            ...
            if (tags.length > MAX_OLD_NUMBERS) {
                fail(`ERROR: Too many ${prefix}build-number- refs in repository, found ${tags.length}, expected only 1. Check your tags!`);
            }
...
function createBuildTags(prefix, nextBuildNumber, callback) {
    let newRefData = {
...
function main() {
    ...
    fetchBuildTags(prefix, (currentBuildNumber, nextBuildNumber, tags) => {
        ...
        if(skipIncrement && nextBuildNumber > 1) {
            ...
            createBuildTags(prefix, nextBuildNumber, (nextBuildNumber) => {
