...
function fetchBuildTags(opts, callback) {
    request('GET', `/repos/${env.GITHUB_REPOSITORY}/git/refs/tags/${opts.buildTag}-`, null, (err, status, result) => {
        ...
        if (status === 404) {
            ...
            const regexString = `/${opts.buildTag}-(\\d+)$`;
            ...
            if (tags.length > MAX_OLD_NUMBERS) {
                fail(`ERROR: Too many ${opts.buildTag} refs in repository, found ${tags.length}, expected only 1. Check your tags!`);
            }
...
function createBuildTags(opts, nextBuildNumber, callback) {
    let newRefData = {
        ref:`refs/tags/${opts.buildTag}-${nextBuildNumber}`,
        sha: env.GITHUB_SHA
    };
...
function main() {
    ...
    const opts = {
        ...
        suffix,
        tagBaseName,
        buildTag: `${prefix}${tagBaseName}${suffix}`
    }
