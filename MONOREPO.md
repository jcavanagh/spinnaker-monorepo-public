...
- Reworking of all versioning and publishing
  ...
  - `release-*` versioning
    - Release branches will build according to their ref
      - e.g. `release-2023.1` generates build versions `2023.1.1` and so forth
...
Three major components are `spinnaker-libraries.yml`, `generic-jvm.yml` and `version.yml`.
...
`generic-jvm.yml` is a reusable workflow for publishing all artifacts required by JVM service projects.
